...
{
	//declaring i
	//made a node which is refencing the first
	int i = 0;
	Node *n = this->first;
...
//********SUBSTITUTE
	void LinkedList::substitute(string newLine, int lineNum)
	{
		...
		//storing our data on the node before the node being replaced
		n2 = n;
		//Incrementing to node to be replaced
		n = n->next;

		//setting the substitution node before this one to point to the new node
		n2->next = newNode;

		//Seting the new node to point to the next node
		newNode->next = n->next;

		//deleting the current node (n)
		delete n;
